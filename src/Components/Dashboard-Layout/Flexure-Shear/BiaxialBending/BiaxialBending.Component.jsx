...
const BiaxialBending = () => {
    ...
    const {
        Fb, Ff, Fr, S, Mr, Fv, An, Vr, ln, Mfx, Mfy, Vf, Qf, Qr, delta_Limit, I, EsI, delta, CB, CK
    } = results;

    const { shearCheck, bendingCheckX, bendingCheckY, deflectionCheck, bearingCheck, shearFractureCheck } = capacityChecks;
    ...
    useEffect(() => {
        ...
        setComputedValues({
            ...
            theta: (parseFloat(roofPitch) / 360) * (2 * Math.PI),
            E: sawnTimber.E(),
            fb: sawnTimber.fb(),
            fv: sawnTimber.fv(),
            fcp: sawnTimber.fcp(),
    ...
    useEffect(() => {
        setResults({
            ...
            // Factored Loading
            ln: beamSpan,
            Mfx: (((factoredLoading * Math.sin(theta) * tributaryWidth * (beamSpan ** 2))) / 8) * (10 ** -9),
            Mfy: (((factoredLoading * Math.cos(theta) * tributaryWidth * (beamSpan ** 2))) / 8) * (10 ** -9),
    ...
    useEffect(() => {
        setCapacityChecks({
            ...capacityChecks,
            shearCheck: Vf / Vr,
            bendingCheckX: Mfx / Mr,
            bendingCheckY: Mfy / Mr,
                        ...
                        <div className="info-card info-card-primary">
                            <div className="card-body">
                                ...
                                <div className="card-text">
                                    ...
                                    <p><Tex>{`M_{fx} = \\dfrac{\\text{ factored loading } \\cdot \\text{ joist spacing } \\cdot l_n^2}{8}  `}</Tex></p>
                                    <p><Tex>{`M_{fx} = ${Mfx.toFixed(2)} \\text{ kN-m }`}</Tex></p>
                                    <p><Tex>{`M_{fy} = \\dfrac{\\text{ factored loading } \\cdot \\text{ joist spacing } \\cdot l_n^2}{8}  `}</Tex></p>
                                    <p><Tex>{`M_{fy} = ${Mfy.toFixed(2)} \\text{ kN-m }`}</Tex></p>
                        ...
                        <div className="info-card info-card-primary">
                            <div className="card-body">
                                ...
                                <div className="card-text">
                                    ...
                                    <p><Tex>{`M_fx = ${Mfx.toFixed(2)} \\text{ kN-m }`}</Tex></p>
                                    <p><Tex>{`M_r = ${Mr.toFixed(2)} \\text{ kNm} `}</Tex></p>
                                    <p><Tex>{`M_fx = ${Mfx.toFixed(2)} \\text{ kN-m }`}</Tex></p>
                                    <p className={bendingCheckX >= 1 ? 'text-danger' : 'text-success'}>
                                        <Tex>{`DC_{ratio} = ${bendingCheckX.toFixed(2)} `}</Tex>
                                        {
                                            bendingCheckX >=1 ? (
